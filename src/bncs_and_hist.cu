...
void bncs_and_hist(const float* h_input, int height, int width, int depth,
    ...
    histogram1D_kernel<<<blocksPerGrid, threadsPerBlock, nbins * sizeof(float),
                         stream1>>>(d_a, d_b, nbins, minvalue, maxvalue,
                                    k2_total_elements);
...
float benchmark_bncs_and_hist(const float* h_input, int height, int width,
    ...
    for (int i = 0; i < 5; i++) {
        ...
        histogram1D_kernel<<<blocksPerGrid, threadsPerBlock,
            ...
            d_a, d_b, nbins, minvalue, maxvalue, k2_total_elements);
        cudaDeviceSynchronize();
    }
