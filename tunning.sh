...
for k1 in 1 2 4 8; do
    for k2 in 1 2 4 8; do
        ...
        for i in 32 64 128 256 512 1024 2048; do
            ...
            mem=$(/usr/local/cuda/bin/ncu --import ./tunning_res/hfuse$i.ncu-rep | grep "Memory \[%\]" | grep -Eo "[0-9\.]+")
            com=$(/usr/local/cuda/bin/ncu --import ./tunning_res/hfuse$i.ncu-rep | grep "Compute (SM) \[%\]" | grep -Eo "[0-9\.]+")
            echo "gridDim[${i}] mempry: ${mem} computation: ${com}" >>$log_file
